...
class FarmerGroupSerializer(serializers.ModelSerializer):
    ...
    def member_count(self, obj):
        m = CooperativeMember.objects.filter(farmer_group=obj)
        return m.count()

    class Meta:
        model = FarmerGroup
        fields = ['id', 'name', 'cooperative', 'union', 'members']
...
class MemberOrderSerializer(serializers.ModelSerializer):
    member = MemberSerializer(read_only=True)
    cooperative = CooperativeSerializer(read_only=True)
    farmer_group = FarmerGroupSerializer(read_only=True)
...
class MemberOrderFormSerializer(serializers.ModelSerializer):
    ...
    #     if member:
    #         try:
    #             member = CooperativeMember.objects.get(member_id=member)
    #             data['member'] = member.id
    #         except ValueError:
    #             raise serializers.ValidationError("Farmer with ID %s not found" % member)
    #     return data
