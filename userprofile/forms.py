...
from coop.models import CooperativeAdmin, Cooperative, FarmerGroup, FarmerGroupAdmin
from product.models import Supplier, SupplierAdmin
...
class UserProfileForm(forms.ModelForm):
    other_cooperative = forms.MultipleChoiceField(required=False,  choices=[])
    farmer_group = forms.MultipleChoiceField(required=False, choices=[])
    supplier = forms.ChoiceField(required=False, choices=[])

    class Meta:
        model = Profile
        fields = ['msisdn', 'access_level', 'role', 'supervisor', 'district',  'farmer_group', 'supplier', 'other_cooperative']
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['other_cooperative'].widget.attrs.update({'class': "selec_adv_1"})

        self.fields['supplier'].choices = [[x.id, x.name] for x in Supplier.objects.all()]
        ...
        self.fields['supplier'].widget.attrs.update({'class': "selec_adv_1"})
